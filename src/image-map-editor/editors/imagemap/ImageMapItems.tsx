...
import { getImageMapPaletteAccent, getImageMapPaletteLabel } from './imagemapPalette.model';
...
const ImageMapItems = React.forwardRef<ImageMapItemsHandle, ImageMapItemsProps>(function ImageMapItems(
	...
	const renderItem = (item: DescriptorItem, centered?: boolean, category = '') => {
		...
		return (
			...
			>
				<span className="rde-editor-items-item-icon">
					<ImageMapObjectIcon type={item.option.type} style={{ ...item.icon.style, color: accent }} />
				</span>
				{collapse ? null : <div className="rde-editor-items-item-text">{item.name}</div>}
			</Button>
		);
	...
	return (
		<div className={className}>
			<Flex className="rde-editor-items-layout" flex="1" flexDirection="column">
				<EditorPanelHeader
					eyebrow={collapse ? undefined : mode === 'assets' ? '创建' : '画布'}
					title={collapse ? null : mode === 'assets' ? '素材库' : '图层'}
					action={
						<CommonButton
